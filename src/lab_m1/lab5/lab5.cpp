...
void Lab5::Init()
{
    ...
    // parameteres for projectionMatrix
    left = 0;
    right = 8;
    bottom = 0;
    top = 5;
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        // Z to decrease FOV (works only on perspective view)
        if (window->KeyHold(GLFW_KEY_Z) && isPerspectiveView && fov > minFOV) {
            float speedModifier = 5; // feels way to slow without this
            fov -= deltaTime * cameraSpeed * speedModifier;
            projectionMatrix = glm::perspective(RADIANS(fov), window->props.aspectRatio, znear, zfar);
        }

        // X to increase FOV (works only on perspective view)
        if (window->KeyHold(GLFW_KEY_X) && isPerspectiveView && fov < maxFOV) {
            float speedModifier = 5; // feels way to slow without this
            fov += deltaTime * cameraSpeed * speedModifier;
            projectionMatrix = glm::perspective(RADIANS(fov), window->props.aspectRatio, znear, zfar);
        }
        ...
        if (window->KeyHold(GLFW_KEY_I) && !isPerspectiveView) {
        ...
        if (window->KeyHold(GLFW_KEY_O) && !isPerspectiveView) {
            right += cameraSpeed * deltaTime;
            left -= cameraSpeed * deltaTime;
            projectionMatrix = glm::ortho(left, right, bottom, top, znear, zfar);
        }
        ...
        if (window->KeyHold(GLFW_KEY_J) && !isPerspectiveView) {
        ...
        if (window->KeyHold(GLFW_KEY_K) && !isPerspectiveView) {
            top += cameraSpeed * deltaTime;
            bottom -= cameraSpeed * deltaTime;
            projectionMatrix = glm::ortho(left, right, bottom, top, znear, zfar);
        }
